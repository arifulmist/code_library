...
int main()
{
    ariful
    ll t=1;
    double m,n,c=0,x,y,z,a,b,d,q,k;
    cin >> t;
    ...
    double dd=0.0;
       dd+=sqrt((pow((v[0].first),2)+pow((v[0].second),2)));
       for(ll i=1;i<t;i++)
    {
       dd+=sqrt(pow((v[i].first-v[i-1].first),2)+pow((v[i].second-v[i-1].second),2));
    }
    dd+=sqrt(pow(v[t-1].first,2)+pow((v[t-1].second),2));
       cout<<fixed<<setprecision(15)<<dd<<"\n";
    shesh
} 
